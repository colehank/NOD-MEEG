...
def _get_meg_pos(epo):
    ...
    meg_layout = mne.channels.find_layout(
        epo.info, ch_type='meg', exclude=exclude_list,
    )
...
def make_main_plot(
    ...
    for ax, (name, comp) in zip([ax1, ax2, ax3], arti_data.items()):
        mne.viz.plot_topomap(
            component_(ica, comp), _get_meg_pos(raw4plot),
    ...
    for artifact, ax in zip(arti_data.keys(), [ax4, ax5, ax6]):
        ica_artifact = sources_(
            ica, raw4plot, arti_data[artifact], tlim=(T_MIN, T_MAX),
        )
    ...
    for meg_data, eeg_data, ax in zip([pre_data, pos_data], [Epre_data, Epos_data], axes):
        ...
        for i in range(len(meg_data[0])):
            ax.plot(
                meg_data[1] - T_MIN, meg_data[0][i] + i *
                offset, color=colors[1] if ax == ax7 else colors[2],
            )
            ax.text(
                meg_data[1][-1] - T_MIN, meg_data[0][i][-1] + i * offset + 3,
        ...
        for i in range(len(eeg_data[0])):
            ax.plot(
                eeg_data[1] - T_MIN, eeg_data[0][i] + eeg_offset +
                i * offset, color=colors[1] if ax == ax7 else colors[2],
            )
            ax.text(
                eeg_data[1][-1] - T_MIN, eeg_data[0][i][-1] +
                eeg_offset + i * offset + 4,
