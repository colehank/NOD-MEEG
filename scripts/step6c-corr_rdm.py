...
def generate_frames(rdms, superclass_map, vmin, vmax, cmap, title):
    ...
    for t in tqdm(range(len(rdms)), desc="Generating frames"):
        frame = generate_frame(t, rdms, superclass_map, vmin, vmax, cmap, title)  # 每个帧生成一次
        frames.append(frame)
    return frames
...
for frames, name in zip([meg_frames_OT, meg_frames_all, eeg_frames_OT, eeg_frames_all], 
...
cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap='seismic'), cax=ax, orientation='vertical', 
                    ticks=[-1, -0.5, 0, 0.5, 1])
...
def plot_superclass_colors(superclass_map, ax=None):
    ...
    color_dict = {superclass: superclass_colors[i] for i, superclass in enumerate(superclasses)}
    ...
    for idx, (superclass, color) in enumerate(color_dict.items()):
        ...
        text = ax.text(col + 0.5, n_rows - 1 - row + 0.5, superclass, ha='center', va='center',
