...
parser.add_argument("--auto", type=str, default="no", choices=["yes", "no"],
...
class ImageLabelerApp:
    def __init__(self, master, img_directory, data_file, sources_directory, properties_directory):
        ...
        # 加载图片
        self.images = [f for f in os.listdir(img_directory) if f.endswith(('png', 'jpg', 'jpeg'))]
        self.images.sort()
    ...
    def setup_ui(self):
        ...
        self.image_label.grid(column=1, row=1, columnspan=2, sticky=(tk.W, tk.E))

        self.filename_label = ttk.Label(self.frame, text="", background='light gray')
        self.filename_label.grid(column=1, row=2, columnspan=2, sticky=(tk.W, tk.E))

        self.text_entry = ttk.Entry(self.frame, width=20)
        self.text_entry.grid(column=1, row=3, columnspan=2, sticky=tk.EW)

        self.progress_label = ttk.Label(self.frame, text="", background='light gray')
        self.progress_label.grid(column=1, row=4, columnspan=2, sticky=(tk.W, tk.E))

        self.next_button = ttk.Button(self.frame, text="Next", command=self.save_label_and_next)
        self.next_button.grid(column=2, row=5, sticky=tk.EW)

        self.prev_button = ttk.Button(self.frame, text="Previous", command=self.go_to_previous_image)
        self.prev_button.grid(column=1, row=5, sticky=tk.EW)

        self.sources_button = ttk.Button(self.frame, text="Sources", command=self.show_source_image)
        self.sources_button.grid(column=1, row=6, sticky=tk.EW)

        self.properties_button = ttk.Button(self.frame, text="Properties", command=self.show_properties_image)
    ...
    def show_image(self, filename):
        ...
        self.filename_label.config(text=f"Current: {filename}", anchor='center')
        self.update_progress_label()
    ...
    def show_source_image(self):
        ...
        pattern = os.path.join(self.sources_directory, f"{self.current_image}*.png")
        ...
        if source_files:
            ...
            for file in source_files:
                ...
                images.append(tk_img)  # Keep a reference to prevent garbage collection
    ...
    def show_properties_image(self):
        i = simpledialog.askinteger("Input", "Please enter the component number:")
        if i is not None:
            path = os.path.join(self.properties_directory, self.current_image, f"{i}.png")
            if os.path.exists(path):
                ...
                messagebox.showwarning("Warning", "No properties image found for this component.")
    ...
    def save_label_and_next(self):
        ...
        if not label_input.strip():
            messagebox.showwarning("Warning", "Please provide a label before moving to the next image.")
            return
        label_list = [int(item) for item in label_input.split(',')] if ',' in label_input else [int(label_input)]
        self.data[self.current_image] = label_list
        with open(self.data_file, 'w') as file:
            json.dump(self.data, file, indent=4)
        self.current_index = (self.current_index + 1) % len(self.all_images)
        if self.current_index == 0:
            messagebox.showinfo("Info", "All images have been labeled. Now will exiting.")
            self.master.destroy()
        else:
            self.show_image(self.all_images[self.current_index])
...
if __name__ == "__main__":
    ...
    if auto:
        ...
        sources_directory = filedialog.askdirectory(title="Select Sources Directory")
        properties_directory = filedialog.askdirectory(title="Select Properties Directory")
    data_file = os.path.join(script_dir, 'Artifact_ICs.json')
    app = ImageLabelerApp(root, img_directory, data_file, sources_directory, properties_directory)
