...
class InfoExtraction():
    def __init__(self, root:str, event_root:str)->None:
        ...
        None
        """
        all_events = {f.split('_')[0][-2:] : f'{event_root}/{f}' 
        ...
        if events_subs_mapping:
            ...
            incorrect_subs = [sub for sub in all_paths if sub not in all_events]
...
class Epoching():
    ...
    def run(self,
        ...
        """
        self.load_data()
        self.align_head(method = align_method) if self.datatype == 'meg' else None
        self.epoching()
    ...
    def load_data(self)->None:
        ...
        self.raws = [mne.io.read_raw_fif(rawp, preload = True) for rawp in self.raw_paths]
...
if __name__ == '__main__':
    ...
    epochor = Epoching(info.get_sub_fp(sub)['events'], 
