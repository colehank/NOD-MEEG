...
def rdm_with_class(
    ...
    superclass_colors = np.array(
        [custom_tab30(
            sup_clss.index(sc)) for sc in sup_clss for _ in superclass_map[sc]]
        )
    ...
    # superclass y轴
    ax1.imshow(superclass_colors[:, np.newaxis], aspect='auto', cmap=ListedColormap(superclass_colors))
    ax1.axis('off')

    # superclass x轴
    ax0.imshow(superclass_colors[np.newaxis, :], aspect='auto', cmap=ListedColormap(superclass_colors))
    ax0.axis('off')
    ...
    if show_colorbar:
        ...
        ax3.text(x=5.5, y=0.5, s='Dissimilarity', ha='center', va='center', 
